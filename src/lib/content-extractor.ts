...
function extractContentFromPage(): PageContent {
  ...
  const keywords = keywordsContent ? keywordsContent.split(',').map(k => k.trim()).filter(Boolean) : undefined;
  ...
  if (!content || content.trim().length < 100) {
    ...
    const scripts = clonedBody.querySelectorAll('script, style, nav, footer, aside, .sidebar, .navigation');
