...
export default function Popup() {
	...
	const isDatabaseConfigError = status.error?.includes(
					...
					<div className="flex items-start gap-2">
						<div
							className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
								isDatabaseConfigError ? "bg-amber-100" : "bg-red-100"
							}`}
						>
							{isDatabaseConfigError ? (
								<svg
						...
						<div className="flex-1">
							<p
								className={`text-sm font-medium ${
									isDatabaseConfigError ? "text-amber-800" : "text-red-600"
								}`}
							>
							...
							<p
								className={`text-xs mt-1 ${
									isDatabaseConfigError ? "text-amber-700" : "text-red-600"
								}`}
							>
								{isDatabaseConfigError
