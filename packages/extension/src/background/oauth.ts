		...
		chrome.identity.launchWebAuthFlow(
			...
			async (redirectedTo) => {
				if (chrome.runtime.lastError) {
					...
					if (
						errorMessage?.includes("not approve") ||
						errorMessage?.includes("denied")
					) {
						return reject(
							new Error(
								"Access denied: Please approve the authorization request",
							),
						);
					}
				...
				if (!redirectedTo) {
					return reject(
						new Error(
							"No redirect URL returned - authorization may have been cancelled",
						),
					);
				}
				...
				try {
					...
					return reject(
						new Error(`Failed to parse redirect URL: ${parseError}`),
					);
				}
			},
		);
