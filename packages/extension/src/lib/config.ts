...
export const config: AppConfig = {
	notion: {
		clientId: getEnvVar(
			...
			"257d872b-594c-805a-9f58-0037c0162612",
		),
		clientSecret: getEnvVar("VITE_NOTION_CLIENT_SECRET"),
		redirectUri: getEnvVar("VITE_NOTION_REDIRECT_URI"),
		serverUrl: getEnvVar("VITE_OAUTH_SERVER_URL", "https://your-oauth-server.com"),
	},
...
export function validateConfig(): { isValid: boolean; errors: string[]; warnings: string[] } {
  ...
  if (!config.notion.clientSecret) {
	...
	if (!config.ai.openaiApiKey) {
		warnings.push(
			"VITE_OPENAI_API_KEY not provided - AI features will be disabled",
		);
	}
