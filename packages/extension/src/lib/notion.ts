...
const BOOKMARK_TEMPLATE_URL = "https://www.notion.so/2659466de76d8071b304f2e6654873bd?v=2659466de76d80a0b18b000c997a014a&source=copy_link";
...
export async function validateDuplicatedTemplate(databaseId: string): Promise<{
	...
	try {
		...
		if (apiError?.code === "object_not_found") {
			return {
				isValid: false,
				error: "Database not found. Please make sure you copied the correct URL and the page is accessible."
			};
		}
		...
		if (apiError?.message?.includes("shared with your integration")) {
			return {
				isValid: false,
				error: "Database is not shared with the bookmark sync integration. Please share the duplicated page with the integration."
			};
		}
 ...
 */
export async function handleOAuthTemplateCallback(accessToken: string, templatedDatabaseId: string): Promise<{
	success: boolean;
	error?: string;
	databaseInfo?: any;
}> {
	...
	try {
		...
		try {
			chrome.notifications.create({
				...
				message: "✅ Your official bookmark template is ready! Perfect formatting and views preserved."
			});
		} catch (notificationError) {
			console.log("Could not show notification:", notificationError);
		}
...
export async function validateOAuthTemplateDatabase(databaseId: string): Promise<{
	...
	try {
		...
		if (apiError?.code === "object_not_found") {
			return {
				isValid: false,
				error: "OAuth-templated database not found. Please ensure the template was duplicated correctly during authorization."
			};
		}
		...
		if (apiError?.message?.includes("shared with your integration")) {
			return {
				isValid: false,
				error: "OAuth-templated database is not accessible. This should not happen with OAuth template duplication."
			};
		}
			...
			if ("properties" in result && result.properties) {
				...
				if (hasBookmarkStructure) {
					...
					const title = "title" in result && result.title ?
						result.title.map((t: any) => t.plain_text || "").join("").trim() :
						"Chrome Bookmarks";
	...
	try {
		...
		try {
			chrome.notifications.create({
				...
				message: "✅ Your duplicated bookmark template is ready! All view settings and formatting preserved."
			});
		} catch (notificationError) {
			console.log("Could not show notification:", notificationError);
		}
...
export async function testTemplateAccess(): Promise<{
	...
	try {
		...
		if (apiError?.code === "object_not_found" || 
			...
			return { 
				accessible: false, 
				error: "Template not shared with integration. Please share the template database with your Notion integration." 
			};
		}
...
export async function createDatabaseFromTemplate(): Promise<{
	...
	try {
		...
		throw new Error(
			`Cannot create workspace page: ${apiError?.message || "Unknown error"}`
		);
	}
...
export async function createDatabaseWithCustomProperties(config: CustomDatabaseConfig): Promise<{
	...
	try {
		...
		throw new Error(
			`Failed to create custom database: ${apiError?.message || "Unknown error"}`
		);
	}
}
...
function validateAndNormalizeDatabaseId(databaseId: string): string {
	...
	if (cleanId.length === 32 && !cleanId.includes("-")) {
		...
		cleanId = `${cleanId.substring(0, 8)}-${cleanId.substring(
			8,
			12,
		)}-${cleanId.substring(12, 16)}-${cleanId.substring(
			16,
			20,
...
async function configureCleanTableView(databaseId: string) {
	...
	try {
		...
		console.log("⚠️ Could not configure clean table view (this is expected with current Notion API limitations):", error);
...
async function findExistingBookmark(databaseId: string, bookmarkId: string, availableProps: any) {
	...
	try {
		...
		console.log(`🔍 Searching for existing bookmark with ID ${bookmarkId} in property ${filterProperty}`);
					...
					? [
							{
								...
								paragraph: {
									rich_text: [
										{ text: { content: bm.content.substring(0, 2000) } },
									],
								},
							},
					  ]
...
async function initializeDatabaseOnce(): Promise<string> {
	...
	if (storage.notion_database_id) {
		...
		try {
			const validDatabaseId = validateAndNormalizeDatabaseId(
				storage.notion_database_id,
			);
			...
			if (notion) {
				try {
					...
					try {
						...
						console.warn(
							"❌ Database exists but is not queryable:",
							apiError?.message,
						);
						if (
					...
					const apiError = error as APIError;
					console.warn(
						"❌ Stored database ID is not accessible:",
						apiError?.message,
					);
					...
					if (
						...
						console.log(
							"🧹 Database not shared with integration, creating a new one...",
						);
						await chrome.storage.local.remove(["notion_database_id"]);
					} else {
						console.warn(
							"❌ Database access failed, creating a new one:",
							error,
						);
					}
			...
			console.warn(
				"❌ Stored database ID is invalid format, creating a new one:",
				error,
			);
		...
		try {
			...
			console.log("ℹ️ No existing OAuth-templated database found, will create new one:", oauthError);
		}
	}
	...
	for (const result of searchResponse.results) {
		if ("title" in result && result.title) {
			const title = result.title
				.map(
					(titlePart: { plain_text?: string; type?: string }) =>
						titlePart.plain_text || "",
				)
				.join("")
				.trim();
...
export async function clearStoredDatabase(): Promise<void> {
	...
	console.log(
		"✅ Database ID cleared. A new database will be created on next sync.",
	);
}
		...
		case 'oauth_template':
			...
			if (!duplicatedDatabaseId) {
				...
				if (detection.isTemplateDatabase && detection.databaseInfo) {
					...
					throw new Error("No OAuth-templated database found. Please reconnect via Notion OAuth with template option selected.");
				}
			} else {
...
async function createDefaultBookmarkDatabase(): Promise<{
	...
	try {
		...
		try {
			...
			console.log("ℹ️ Database created successfully, attempting post-creation view configuration...", viewCheckError);
			...
			try {
				...
				console.log("💡 Using property naming optimization (_sync_id) for minimal visibility", fallbackError);
			}
		}
		...
		try {
			...
			throw new Error(
				`Database created but not accessible: ${
					apiError?.message || "Unknown error"
				}`,
			);
		}
		...
		for (const [propName, propType] of Object.entries(REQUIRED_PROPERTIES)) {
			if (!existingProperties[propName]) {
				...
				if (existingProp.type !== propType) {
					validation.issues.push(`Property ${propName} has wrong type: expected ${propType}, found ${existingProp.type}`);
					validation.isCompatible = false;
				}
			}
		}
...
async function handleSchemaMismatch(validation: SchemaValidation): Promise<'migrate' | 'create_new' | 'abort'> {
	...
	if (hasMissingProperties && hasOnlyMinorIssues) {
		...
		try {
			chrome.notifications.create({
				...
				message: "Updating your Notion database to support new features (bookmark paths). This may take a moment..."
			});
		} catch (notificationError) {
			console.log("Could not show notification:", notificationError);
		}
	...
	try {
		chrome.notifications.create({
			...
			message: "Your existing Notion database is incompatible. Creating a new optimized database for bookmarks..."
		});
	} catch (notificationError) {
		console.log("Could not show notification:", notificationError);
	}
...
async function migrateDatabaseSchema(databaseId: string, missingProperties: string[]): Promise<void> {
	if (!notion) {
		throw new Error("Notion client not initialized");
	}
...
async function configureDefaultViewToHideInternalProperties(databaseId: string): Promise<void> {
	...
	try {
		...
		console.log("💡 The '_sync_id' column uses an underscore prefix to be less prominent in the interface");
