...
function main() {
  ...
  try {
    ...
    const serverCoreResult = run(
      ...
      [
        '-e',
        "const core = require('@bookmark-assistant/server-core'); const desc = core.extractDescriptionFromHtml('<html><head><meta name=\"description\" content=\"Runtime package description\" /></head></html>'); const normalized = core.normalizeBookmarkForSyncPlanning({ title: 'Reading', url: 'https://example.com/read', source: 'reading_list', type: 'reading_list', readState: 'READ', tags: ['runtime'] }); const unsynced = core.selectUnsyncedDescribedBookmarks([{ title: 'Existing', url: 'https://example.com/existing', description: 'Existing description' }, { title: 'New', url: 'https://example.com/new', syncId: 'new-sync-id', description: 'New description' }], ['https://example.com/existing'], []); const notionProps = core.buildBookmarkPropertiesFromNotionSchema({ Name: { type: 'title' }, URL: { type: 'url' }, Type: { type: 'single_select' }, Status: { type: 'status' }, Site: { type: 'formula' } }, { title: 'Runtime Notion', url: 'https://example.com/notion', type: 'reading_list', readState: 'READ' }); const notionKeys = core.extractNotionPageLinkKeys({ properties: { URL: { type: 'url', url: 'https://example.com/runtime' }, 'Sync ID': { type: 'rich_text', rich_text: [{ plain_text: 'runtime-sync' }] } } }); const notionFolderTags = core.extractNotionPageFolderAndTags({ properties: { Folder: { type: 'rich_text', rich_text: [{ text: { content: 'Runtime Folder' } }] }, Tags: { type: 'multi_select', multi_select: [{ name: 'runtime' }, { name: 'core' }] } } }); const notionTimestamps = core.extractNotionPageTimestamps({ created_time: '2026-01-01T00:00:00.000Z', last_edited_time: '2026-01-02T00:00:00.000Z', properties: {} }); const retryable = core.selectRetryableSyncFailures([{ success: false, bookmark: 'Retry', error: 'fetch failed', retryCount: 0 }, { success: false, bookmark: 'No retry', error: 'Notion API error', retryCount: 0 }]); const merged = core.mergeRetryResults([{ success: false, bookmark: 'Retry', error: 'fetch failed', retryCount: 0 }], [{ success: true, bookmark: 'Retry', action: 'created', retryCount: 1 }]); console.log([typeof core.diffBookmarks, typeof core.validateLinkItemInput, typeof core.normalizeBookmarkForSyncPlanning, normalized.source, normalized.type, normalized.readState, normalized.tags.join('|'), typeof core.normalizeUrlForSync, core.normalizeUrlForSync('https://example.com/page/?b=2&a=1#top'), core.isValidUrl('file:///tmp/page.html'), core.isFetchableHttpUrl('file:///tmp/page.html'), typeof core.extractDescriptionFromHtml, desc.text, desc.source, core.sanitizeDescription('<p>Text &amp; content</p>'), typeof core.selectUnsyncedDescribedBookmarks, unsynced.length, unsynced[0].syncId, typeof core.buildBookmarkPropertiesFromNotionSchema, notionProps.Name.title[0].text.content, notionProps.Type.single_select.name, notionProps.Status.status.name, core.isReadOnlyNotionPropertyType('formula'), Object.prototype.hasOwnProperty.call(notionProps, 'Site'), typeof core.extractNotionPageLinkKeys, notionKeys.url, notionKeys.syncId, typeof core.extractNotionPageFolderAndTags, notionFolderTags.folder, notionFolderTags.tags.join('|'), typeof core.extractNotionPageTimestamps, notionTimestamps.createdTime, notionTimestamps.lastEditedTime, typeof core.buildGoogleS2FaviconUrl, core.buildGoogleS2FaviconUrl('https://docs.example.com/path', { size: 128 }), core.buildGoogleS2FaviconUrl('not-a-valid-url') === undefined, typeof core.isRetryableSyncError, core.isRetryableSyncError('read ECONNRESET'), retryable.length, merged[0].success, typeof core.isRetryablePersistenceError, core.isRetryablePersistenceError({ code: 'P2024' }), core.isRetryablePersistenceError({ message: 'Unique constraint failed' }), typeof core.getPrimaryNotionDataSourceId, core.getPrimaryNotionDataSourceId({ data_sources: [{ id: 'ds-runtime' }] }), core.getPrimaryNotionDataSourceId({ data_sources: [] }) === undefined, typeof core.hasNotionDataSourceId, core.hasNotionDataSourceId({ data_sources: [{ id: 'ds-runtime' }] }, 'ds-runtime'), core.hasNotionDataSourceId({ data_sources: [{ id: 'ds-runtime' }] }, 'ds-missing')].join(','))",
      ],
    ...
    if (
      serverCoreResult !==
      'function,function,function,reading_list,reading_list,READ,runtime,function,https://example.com/page?a=1&b=2,true,false,function,Runtime package description,meta_description,Text content,function,1,new-sync-id,function,Runtime Notion,Reading List,Read,true,false,function,https://example.com/runtime,runtime-sync,function,Runtime Folder,runtime|core,function,2026-01-01T00:00:00.000Z,2026-01-02T00:00:00.000Z,function,https://www.google.com/s2/favicons?domain=docs.example.com&sz=128,true,function,true,1,true,function,true,false,function,ds-runtime,true,function,true,false'
    ) {
      fail(
        `Expected server-core diff, validation, normalization, URL, description, sync planning, Notion property, Notion page extraction, database, favicon, retry, and persistence helpers to be available, got ${serverCoreResult}`
      );
    }
